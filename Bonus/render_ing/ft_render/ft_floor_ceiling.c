...
/*   Updated: 2026/03/11 19:45:54 by mdakni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_floor_ceiling3(t_cube *cube, t_vars9 *vars)
{
	...
	if (vars->tmp > 1.0)
		...
		vars->tmp = 0.0;
	vars->r = (vars->tex->pixels[vars->k + 0] * vars->tmp) + (0x33 * (1.0
				- vars->tmp));
	vars->g = (vars->tex->pixels[vars->k + 1] * vars->tmp) + (0x33 * (1.0
				- vars->tmp));
	vars->b = (vars->tex->pixels[vars->k + 2] * vars->tmp) + (0x33 * (1.0
				- vars->tmp));
